...
locals {
  github-email    = "xxxxxxxxxxxxxxxx"   # you need to change this line
  github-username = "xxxxxxxxxx"               # you need to change this line
  github-token    = "xxxxxxxxxxxxxxxxxxxxxxxxxx"        # you need to change this line
  key_pair        = "xxxxxxxxxxxx"              # you need to change this line
  pem_key_address = "xxxxxxxxxx" # you need to change this line
  user            = "xxxxxxx"
}
...
resource "aws_instance" "jenkins-server" {
  ...
  provisioner "local-exec" {
    command = "ssh -i ${local.pem_key_address} -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@${aws_instance.jenkins-server.public_ip} 'sudo cat /var/lib/jenkins/secrets/initialAdminPassword' > initialpasswd.txt"
  }
